...
public class FileService {
    ...
    @Autowired
    FilePortfolioMapperRepository filePortfolioMapperRepository;
    ...
    public FileEntity uploadPdf(MultipartFile[] files, String filepath, long pfId){
        String FileNames = "";
        ...
        final FilePortfolioMapper filePortfolioMapper = FilePortfolioMapper.builder()
                .fId(fid)
                .pfId(pfId)
                .build();
        Long fpmId = filePortfolioMapperRepository.save(filePortfolioMapper).getId();
        log.info("create FilePortfolio is +" + fpmId + ", fid : " + fid + " pfId : " + pfId);

        return file;
    }

    public Resource getPdf(String path, long pfId) throws IOException {
        long fId = filePortfolioMapperRepository.findByPfId(pfId).get().getFId();
